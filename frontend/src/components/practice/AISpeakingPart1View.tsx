...
export default function AISpeakingPart1View({ exercise, onBack }: Props) {
  ...
  const submitFinalRecording = async (blob: Blob) => {
    ...
    try {
      ...
      progressAPI.createSession({
        ...
        notes: `Speaking Part 1 Interview — Band ${overallBand}`,
      }).catch(() => {});
    } catch (err: any) {
    ...
    return (
      <div className="p1-container">
        <div className="p1-intro">
          ...
          <p className="p1-intro-desc">The examiner will ask you questions about 3 topics. Answer each question in 15-30 seconds.</p>
          <div className="p1-topics-preview">
            {topics.map((t, i) => (
              <div key={i} className="p1-topic-chip">{t.area}</div>
            ))}
          </div>
          <p className="p1-intro-count">{totalQ} questions · ~5 minutes</p>
